...
# ###################################################################
# Rates Request Class for Fedex International Shipments #############
# ###################################################################
class FedexInternationalRateServiceRequest(FedexBaseService):
    """
    This class allows you to get the shipping charges for a particular address.
    You will need to populate the data structures in self.RequestedShipment,
    then send the request.
    ...
    def __init__(self, config_obj, *args, **kwargs):
        ...
        """@ivar: Holds the RequestedShipment WSDL object including the shipper, recipient and shipt time."""
        # Call the parent FedexBaseService class for basic setup work.
        super(FedexInternationalRateServiceRequest, self).__init__(
            self._config_obj, 'RateService_v18.wsdl', *args, **kwargs)
        self.ClientDetail.Region = config_obj.express_region_code
        """@ivar: Holds the express region code from the config object."""
    ...
    def _prepare_wsdl_objects(self):
        ...
        # International options - CustomsClearanceDetail
        customs_clearance_detail = self.client.factory.create('CustomsClearanceDetail')
        ...
        duty_payment= self.client.factory.create('Payment')
        duty_payment.Payor = payor
        duty_payment.PaymentType = 'SENDER'
        customs_clearance_detail.DutiesPayment = duty_payment
